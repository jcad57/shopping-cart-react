...
export default function Cart({
  ...
  return (
    <div className="cart">
      <h2 className="cart-heading">Your Cart ({cartQty})</h2>
      {orderTotal > 0 ? (
        <>
          {cart.map((item) => (
            <CartItem item={item} handleRemoveItem={handleRemoveItem} key={item.id} USDollar={USDollar} />
          ))}
          <div className="order-total">
            <p className="order-total-text">Order Total</p>
            <span className="order-total-amount">{USDollar.format(orderTotal)}</span>
          </div>
          ...
          <button className="confirm-order-btn" onClick={() => setShowOrderConfirm(true)}>
            Confirm Order
          </button>
        </>
      ) : (
        <div className="empty-cart-container">
          ...
          <span className="empty-cart-text">Your added items will appear here</span>
        </div>
      )}
    </div>
  );
