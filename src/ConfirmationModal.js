...
export default function ConfirmationModal({ cart, startNewOrder, orderTotal, USDollar }) {
  return (
    <>
      ...
      <p className="confirmation--heading">Order Confirmed</p>
      <span className="confirmation--message">We hope you enjoy your food!</span>
      <div className="confirmation--order-container">
        {cart.map((item) => (
          <CartItemModal item={item} key={item.id} USDollar={USDollar} />
        ))}

        <div className="confirmation--order-total">
          <span className="confirmation--total-text">Order Total</span>
          <span className="confirmation--total-amount">{USDollar.format(orderTotal)}</span>
        </div>
      </div>
